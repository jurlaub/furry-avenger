...
# output: filename.tour ; 1st line "tour length: #" ; list of city ids
#
def print_to_file_test(filename, ClusterList):
    ...
    for index in Cluster_Sorted:
        ...
        for runner in ClusterList:
            ...
            if m2 < max_neighbor:
                ...
                #print ">>runner: parent: %s" % str(r)
                #print "n_count:N_TEST >> %s" % str(n_count) +str(N_TEST)
...
def cluster_city_bruteforce(cluster, startPoint):
    ...
    if startPoint is not None:
        ...
        tmp[2] = 0
        citiesSorted.append(tmp)
        #print ">> in bruteforce: (the else) " + str(tmp)
    ...
    if Cluster_Sorted:
        if len(Cluster_Sorted)>= 2:
            ...
            for cluster in Cluster_Sorted[1:]:
                ...
                #print "in seqencer: lastcity: " + str(cityTuple)
...
def reorder_city_distance(cluster, entry):
    ...
    if cities:
        ...
        for city in cities:
            ...
            # set new x1, y1
            x1, y1 = x2, y2
            running_total = running_total + m2
            #print "reorder: m2: rt:" + str(m2) +  " " + str(running_total)
...
def input_coords_from_Cluster(Cluster_Sorted, n):
    """
    input: list of clusters, size of new clusters
    outputs: new list of clusters - that have distance n from parent nodes

    >>cites are iterated through
    >>>if within n of a cluster in current list, append
    >>>if not within n, make new cluster with city as parent
    ...
    # obtain starter city to make new cluster
    if Cluster_Sorted:
        tmp = Cluster_Sorted[-1]
        tmp_cities = tmp.getCityList()
            
        entry = tmp_cities.pop()
        entry = entry[0]  # gets tuple 

        #print "input_coords_from_Cluster: what is tmp_cities?: %s" % tmp_cities

        if tmp_cities:
            Cluster_Sorted[-1].replaceCityList(tmp_cities)
            #print "input_coords_from_Cluster: tmp_cities rplaced last cluster cites: %s" % tmp_cities
        else:
            zz = Cluster_Sorted.pop()

        #print "this is the first city: %s" % str(entry)
...
def command(filename):
    ...
    # n_max = int(n_max/2)
    # print n_max
    # ClusterList = input_coords_from_Cluster(ClusterList, n_max)
...
def main():
    ...
    for o, a in opts:
        if o in ("-h", "--help"):
            ...
            print "<filename> is the name of the file to test: on each line:  <city-id  x  y>"
            sys.exit(0)
    # process args
    for arg in args:
